...
class ViewTask extends BakeTask {
     ...
     * @var array
     */
    public $noTemplateActions = array('delete', 'autocomplete');
    ...
    /**
     * Ignorowane od tlumaczen pola w tabeli
     * @var type 
     */
    public $ignoreLocateFields = array('id', 'created', 'deleted', 'img', 'image', 'file', 'name', 'title');
     ...
     * @return mixed
     */
    public function febExecute($connection, $controllerName) {
        parent::execute();
    ...
    /**
     * Handles do metody Febowskiego wypiekania
     *
     * @return void
     */
    protected function _febInteractive() {
        $this->hr();
        $this->out(sprintf("Widoki bakera\nPath: %s", $this->path));
        $this->hr();
        ...
        //Tworze plik tłumaczeń

        foreach ($vars['fields'] as $field) {
            if (!in_array($field, $this->ignoreLocateFields)) {
                $vars['localeFields'][] = $field; 
            }
        }

        $content = $this->getContent('locale', $vars);
        $this->bake($this->controllerName, $content, 'pol' . DS . 'LC_MESSAGES', current(App::path('Locale')));

        //Tworze element form.ctp dla modelu
        $content = $this->getContent('fields', $vars);
        $this->bake('fields', $content, 'Elements');

        $content = $this->getContent('table_index', $vars);
        $this->bake('table_index', $content, 'Elements');

        $this->hr();
        $this->out(__d('cake_console', "Widoki wypieczone.\n"));
        $this->hr();
    }
     ...
     */
    public function bake($action, $content = '', $elementPath = '', $fullPath = null) {
        if ($content === true) {
        ...
        if (!$fullPath) {
            $path = $this->getPath();
            $filename = $path . $elementPath . DS . $this->controllerName . DS . Inflector::underscore($action) . '.ctp';
        } else {
            $filename = $fullPath . $elementPath . DS . $this->controllerName . '.po';
        }
