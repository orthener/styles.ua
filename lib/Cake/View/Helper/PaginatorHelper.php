	...
	protected function _pagingLink($which, $title = null, $options = array(), $disabledTitle = null, $disabledOptions = array()) {
		...
		foreach (array_keys($_defaults) as $key) {
			...
			unset($options[$key]);
                ...
                //debug($this->request->params['type']);
                if(!empty($this->request->params['type'])) {
                    $url = array_merge(array('type' => $this->request->params['type'],'page' => $paging['page'] + ($which == 'Prev' ? $step * -1 : $step)), $url);
                } else {
	...
	public function numbers($options = array()) {
		...
		if ($params['pageCount'] <= 1) {
			return false;
                ...
                $params['type'] = (!empty($this->request->params['type'])?$this->request->params['type']:'');
		extract($options);
		...
		if ($modulus && $params['pageCount'] > $modulus) {
			...
			for ($i = $start; $i < $params['page']; $i++) {
				$out .= $this->Html->tag($tag, $this->link($i, array('page' => $i, 'type' => (!empty($this->request->params['type'])?$this->request->params['type']:'')), $options), compact('class')) . $separator;
			}
			...
			if ($end != $params['page']) {
				$out .= $this->Html->tag($tag, $this->link($i, array('page' => $end, 'type' => (!empty($this->request->params['type'])?$this->request->params['type']:'')), $options), compact('class'));
			}
			...
			for ($i = 1; $i <= $params['pageCount']; $i++) {
				if ($i == $params['page']) {
					...
					$out .= $this->Html->tag($tag, $this->link($i, array('page' => $i, 'type' => (!empty($this->request->params['type'])?$this->request->params['type']:'')), $options), compact('class'));
				}
				if ($i != $params['pageCount']) {
					$out .= $separator;
				}
	...
	public function last($last = 'last >>', $options = array()) {
		...
		if (is_int($last) && $params['page'] <= $lower) {
			...
			$out = $before . $this->Html->tag(
				$tag, $this->link($last, array('page' => $params['pageCount'], 'type' => (!empty($this->request->params['type'])?$this->request->params['type']:'')), $options), compact('class')
			);
		}
		return $out;
	}
