	...
	protected function _addJoin(Model $model, $query, $field, $aliasField, $locale, $addField = false) {
		...
		if (is_array($locale)) {
			foreach ($locale as $_locale) {
				$model->virtualFields['i18n_' . $model->alias . '_' . $field . '_' . $_locale] = 'I18n__' . $model->alias . '_' . $field . '__' . $_locale . '.content';
				if (!empty($query['fields']) && is_array($query['fields'])) {
					$query['fields'][] = 'i18n_' . $model->alias . '_' . $field . '_' . $_locale;
				}
				$query['joins'][] = array(
					'type' => 'LEFT',
					'alias' => 'I18n__' . $model->alias . '_' . $field . '__' . $_locale,
					'table' => $joinTable,
					'conditions' => array(
						$model->alias . '.' . $model->primaryKey => $db->identifier("I18n__{$model->alias}_{$field}__{$_locale}.foreign_key"),
						'I18n__' . $model->alias . '_' . $field . '__' . $_locale . '.model' => $model->name,
						'I18n__' . $model->alias . '_' . $field . '__' . $_locale . '.' . $RuntimeModel->displayField => $aliasField,
						'I18n__' . $model->alias . '_' . $field . '__' . $_locale . '.locale' => $_locale
					)
				);
			}
		} else {
			$model->virtualFields['i18n_' . $model->alias . '_' . $field] = 'I18n__' . $model->alias . '_' . $field . '.content';
			if (!empty($query['fields']) && is_array($query['fields'])) {
				$query['fields'][] = 'i18n_' . $model->alias . '_' . $field;
			}
			$query['joins'][] = array(
				'type' => 'INNER',
				'alias' => 'I18n__' . $model->alias . '_' . $field,
				'table' => $joinTable,
				'conditions' => array(
					$model->alias . '.' . $model->primaryKey => $db->identifier("I18n__{$model->alias}_{$field}.foreign_key"),
					'I18n__' . $model->alias . '_' . $field . '.model' => $model->name,
					'I18n__' . $model->alias . '_' . $field . '.' . $RuntimeModel->displayField => $aliasField,
					'I18n__' . $model->alias . '_' . $field . '.locale' => $locale
				)
			);
		}
		return $query;
	...
	public function afterFind(Model $model, $results, $primary) {
		...
		foreach ($results as $key => &$row) {
			...
			foreach ($beforeFind as $_f => $field) {
				...
				if (is_array($locale)) {
					foreach ($locale as $_locale) {
						if (!isset($row[$model->alias][$aliasField]) && !empty($row[$model->alias]['i18n_' . $model->alias . '_' . $field . '_' . $_locale])) {
							$row[$model->alias][$aliasField] = $row[$model->alias]['i18n_' . $model->alias . '_' . $field . '_' . $_locale];
							$row[$model->alias]['locale'] = $_locale;
						}
						unset($row[$model->alias]['i18n_' . $model->alias . '_' . $field . '_' . $_locale]);
					}
					...
					$value = '';
					if (!empty($row[$model->alias]['i18n_' . $model->alias . '_' . $field])) {
						$value = $row[$model->alias]['i18n_' . $model->alias . '_' . $field];
					}
					$row[$model->alias][$aliasField] = $value;
					unset($row[$model->alias]['i18n_' . $model->alias . '_' . $field]);
				}
			}
		}
		return $results;
