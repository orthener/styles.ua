...
class CommentBehavior extends TreeBehavior {
...
// 	);
    ...
    function findTree(&$model, $conditions = array()) {
        ...
        $params = array(
            ...
            'order' => "{$model->alias}.{$this->settings[$model->alias]['left']} ASC"
        );
        $findTree = $model->find('threaded', $params);
        return $findTree;
    }
    ...
    function moveNode(&$model, $id, $destination_id = null, $mode = null) {
        ...
        $model->query("UPDATE {$model->useTable} AS {$model->alias} SET {$lft} = -{$lft}, {$rght} = - {$rght} WHERE
            {$lft} >= {$node[$model->alias][$lft]} AND {$rght} <= {$node[$model->alias][$rght]} AND $scope;");
        ...
        if ($mode == 'before' OR $mode == 'top') {
            ...
            if ($shift > 0) {
                ...
                $model->query("UPDATE {$model->useTable} AS {$model->alias} SET {$lft} = {$lft}+{$subtree_size} WHERE
                    {$lft} >= {$destination_node[$model->alias][$lft]} AND {$lft} < {$node[$model->alias][$lft]} AND $scope ;");
                $model->query("UPDATE {$model->useTable} AS {$model->alias} SET {$rght} = {$rght}+{$subtree_size} WHERE
                    {$rght} > {$destination_node[$model->alias][$lft]} AND {$rght} < {$node[$model->alias][$lft]} AND $scope ;");

                //move backward all lft and rght values of node and its subtree
                $model->query("UPDATE {$model->useTable} AS {$model->alias} SET {$lft} = -{$lft}-{$shift}, {$rght} = -{$rght}-{$shift} WHERE
                    {$lft} < 0 AND {$rght} < 0 AND $scope ;");
            } else {
                ...
                $model->query("UPDATE {$model->useTable} AS {$model->alias} SET {$lft} = {$lft}-{$subtree_size} WHERE
                    {$lft} < {$destination_node[$model->alias][$lft]} AND {$lft} > {$node[$model->alias][$rght]} AND $scope ;");
                $model->query("UPDATE {$model->useTable} AS {$model->alias} SET {$rght} = {$rght}-{$subtree_size} WHERE
                    {$rght} < {$destination_node[$model->alias][$lft]} AND {$rght} > {$node[$model->alias][$rght]} AND $scope ;");
            ...
            if ($shift > 0) {
                ...
                //move forward all lft, rght values between old and new node location 
                $model->query("UPDATE {$model->useTable} AS {$model->alias} SET {$lft} = {$lft}+{$subtree_size} WHERE
                    {$lft} > {$destination_node[$model->alias][$rght]} AND {$lft} < {$node[$model->alias][$lft]} AND $scope ;");
                $model->query("UPDATE {$model->useTable} AS {$model->alias} SET {$rght} = {$rght}+{$subtree_size} WHERE
                    {$rght} > {$destination_node[$model->alias][$rght]} AND {$rght} < {$node[$model->alias][$lft]} AND $scope ;");
                ...
                //move backward all lft, rght values between old and new node location 
                $model->query("UPDATE {$model->useTable} AS {$model->alias} SET {$lft} = {$lft}-{$subtree_size} WHERE
                    {$lft} < {$destination_node[$model->alias][$rght]} AND {$lft} > {$node[$model->alias][$rght]} AND $scope ;");
                $model->query("UPDATE {$model->useTable} AS {$model->alias} SET {$rght} = {$rght}-{$subtree_size} WHERE
                    {$rght} <= {$destination_node[$model->alias][$rght]} AND {$rght} > {$node[$model->alias][$rght]} AND $scope ;");
        ...
        $model->query("UPDATE {$model->useTable} AS {$model->alias} SET {$parent} = {$parent_node} WHERE
