...
function updateDragBox() {
    ...
    menuName.keyup(function() {
        updateNameNewLi(draggable,menuName);
    });
    menuName.blur(function() {
        updateNameNewLi(draggable,menuName);
    });
}
function updateNameNewLi(draggable,menuName){
    jQuery(draggable.find("span:first")).html(menuName.val());
        uiHidden();
}

function uiHidden(){
    valInput = jQuery("#MenuName").val();
    draggable = jQuery(".ui-hidden");
     if(valInput < 1){
