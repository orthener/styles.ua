...
class ControllerTask extends BakeTask {
    ...
    public function febExecute($connection, $controllerName) {
        ...
        if (empty($this->args)) {
            return $this->_febInteractive();
        }
    }

    /**
     * Metoda tworząca kontroller w standardzie Fabryki e-Biznesu 
     *
     * @return void
     */
    protected function _febInteractive() {
        ...
        $this->hr();
        $this->out(__d('cake_console', 'Pieczemy %sController', $this->controllerName));
        $this->hr();
        ...
        //Szybkie tworzenie kontrolera        
        $actions = $this->bakeActions($this->controllerName, null, true);
        $admin = $this->Project->getPrefix();
        $actions .= $this->bakeActions($this->controllerName, $admin, true);

        $baked = $this->bake($this->controllerName, $actions, $helpers, $components);
    ...
    public function doComponents() {
        return $this->_doPropertyChoices(
                        __d('cake_console', "Would you like this controller to use any components?"), __d('cake_console', "Please provide a comma separated list of the component names you'd like to use.\nExample: 'Acl, Security, RequestHandler'")
        );
    }
