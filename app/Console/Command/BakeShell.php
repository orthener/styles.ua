 ...
 */
	public function main() {
		...
		$this->out(__d('cake_console', '[Fi]xture'));
		$this->out(__d('cake_console', '[T]est case'));
		$this->out(__d('cake_console', '[F]eb Baker'));
		$this->out(__d('cake_console', '[Q]uit'));

		$classToBake = strtoupper($this->in(__d('cake_console', 'Co dzis pieczemy?'), array('D', 'M', 'V', 'C', 'P', 'F', 'T', 'Q')));
		switch ($classToBake) {
			case 'D':
			...
			case 'T':
				...
				break;
			case 'F':
				$this->Model->febExecute();			
                $this->Controller->febExecute($this->Model->connection, Inflector::pluralize($this->Model->currentModelName));
				$this->View->febExecute($this->Model->connection, Inflector::pluralize($this->Model->currentModelName));
				break;
			case 'Q':
				exit(0);
				break;
			default:
				$this->out(__d('cake_console', 'You have made an invalid selection. Please choose a type of class to Bake by entering D, M, V, Fi, T, F or C.'));
		}
		$this->hr();
		$this->main();
	}
