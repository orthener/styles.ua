...
class ModelTask extends BakeTask {
     ...
     * @var array
     */
    public $skipTables = array('i18n', 'categories', 'helps', 'newsletters', 'newsletter_messages', 'pages', 'page_photos', 'permissions', 'requesters_permissions', 'settings', 'slugs', 'users', 'users_logs', 'groups', 'groups_users', 'comments');
    ...
    /**
     * Execution method always used for tasks dla Feb
     *
     * @return mixed
     */
    public function febExecute() {
        parent::execute();
        if (empty($this->args)) {
            $this->_febInteractive();
        }
    }
     ...
     * @return boolean
     */
    protected function _febInteractive() {
        $this->hr();
        $this->out(__d('cake_console', "Pieczemy model\nPath: %s", $this->path));
        $this->hr();
        $this->interactive = true;
        ...
        $validate = $this->doFebValidation($tempModel);
                
        $associations = $this->doFebAssociations($tempModel);
        

        $vars = compact('associations', 'validate', 'primaryKey', 'useTable', 'displayField');
        $vars['useDbConfig'] = $this->connection;
        
        $vars['fields'] = $tempModel->schema();
        
        $this->bake($currentModelName, $vars);
    ...
    /**
     * Handles Generation and user interaction for creating validation. Febowska
     *
     * @param Model $model Model to have validations generated for.
     * @return array $validate Array of user selected validations.
     */
    public function doFebValidation($model) {
        if (!is_object($model)) {
            return false;
        }
        $fields = $model->schema();
        ...
        foreach ($fields as $fieldName => $field) {
            $validation = $this->fieldFebValidation($fieldName, $field, $model->primaryKey);
            if (!empty($validation)) {
                if (isSet($validation['none'])) continue;
                $validate[$fieldName] = $validation;
            }
        }
    ...
    /**
     * Does individual field validation handling. Febowska
     *
     * @param string $fieldName Name of field to be validated.
     * @param array $metaData metadata for field
     * @param string $primaryKey
     * @return array Array of validation for the field.
     */
    public function fieldFebValidation($fieldName, $metaData, $primaryKey = 'id') {
        $defaultChoice = count($this->_validations);
        $validate = $alreadyChosen = array();
        ...
        $methods = array_flip($this->_validations);
        $guess = $defaultChoice;
        if (($metaData['null'] != 1 && !in_array($fieldName, array($primaryKey, 'created', 'modified', 'updated'))) || in_array($fieldName, array('img', 'image', 'file'))) {
            if ($fieldName == 'email') {
                $guess = $methods['email'];
            } elseif ($metaData['type'] == 'string' && $metaData['length'] == 36) {
                $guess = $methods['uuid'];
        ...
        if (isset($this->_validations[$guess])) {
            $validatorName = $this->_validations[$guess];
        } else {
            $validatorName = Inflector::slug($guess);
        }
            
        if (is_numeric($guess) && isset($this->_validations[$guess])) {
            $validate[$validatorName] = $this->_validations[$guess];
        } else {
            $validate[$validatorName] = $guess;
        }
    ...
    /**
     * Handles associations Febowska
     *
     * @param Model $model
     * @return array $assocaitons
     */
    public function doFebAssociations($model) {
        if (!is_object($model)) {
            return false;
        }
    ...
    */
    public function bake($name, $data = array()) {
        ...
        $this->out("\n" . __d('cake_console', 'Pieke model dla klasy %s...', $name), 1, Shell::QUIET);
