		...
		$this->controller->set(array(
			...
			'_serialize' => array('name', 'url')
                ...
                if ($error->getCode() == 404) {
                    $this->controller->loadModel('Page.Page');
                    $page = $this->controller->Page->find('first', array(
                        'conditions' => array('Page.id' => 63)
                    ));
                    if (!empty($page)) {
                        $this->controller->redirect(array('admin' => false, 'plugin' => 'page', 'controller' => 'pages', 'action' => 'view', $page['Page']['slug']));
                    }
                }
