            ...
            <div class="row-fluid ff-container">
                <div class="span12 pull-right">
                    <ul class="thumbnails ff-items">
                        ...
                        //$files = scandir(WWW_ROOT . $cur_dir);

                        foreach ($files as $file) {
                            if (is_dir($root . $cur_dir . $file) && $file != '.') {
//                            if (is_dir(WWW_ROOT . $cur_dir . $file) && $file != '.') {
                                //add in thumbs folder if not exist 
                                if (!file_exists("thumbs/" . $subdir . $file))
                                    create_folder(false, "thumbs/" . $subdir . $file);
                                if (($i + $k) % 6 == 0 && $i + $k > 0 && $end == false) {
                                    $end = true;
                                    ?></div><div class="space10"></div><?php
                            } else {
                                $end = false;
                            }
                            if (($i + $k) % 6 == 0 && $start == false) {
                                $start = true;
                                ?>
                            ...
                            <?php if ($file != "..") { ?>
                                        <a href="<?php if ($delete_folder) { ?>dialog.php?del_folder=<?php echo $file; ?>&type=<? echo $_GET['type'] ?>&editor=<?php echo $_GET['editor'] ? $_GET['editor'] : 'mce_0'; ?>&lang=<?php echo $_GET['lang'] ? $_GET['lang'] : 'en_EN'; ?>&fldr=<?php echo $subdir ?><?php } else {
                    echo '#';
                } ?>" class="btn erase-button top-right <?php if (!$delete_folder) echo 'disabled'; ?>" <?php if ($delete_folder) { ?>onclick="return confirm('<?php echo lang_Confirm_Folder_del; ?>');"<?php } ?> title="<?php echo lang_Erase ?>"><i class="icon-trash"></i></a>
                            <?php } ?>
                            ...
                            if (in_array($file_ext, $ext)) {
                                if (in_array($file_ext, $ext_img)) {
                                    ...
                                    if (!file_exists($thumb_path))
                                        create_img_gd(dirname(__FILE__) . DIRECTORY_SEPARATOR . $current_path . $subdir . $file, $thumb_path, 156, 78);
                                    $is_img = true;
                                ...
                                <?php } ?>
                                                    <a href="<?php if ($delete_file) { ?>dialog.php?del_file=<?php echo $file; ?>&type=<?php echo $_GET['type'] ?>&editor=<?php echo $_GET['editor'] ? $_GET['editor'] : 'mce_0'; ?>&lang=<?php echo $_GET['lang'] ? $_GET['lang'] : 'en_EN'; ?>&fldr=<?php echo $subdir ?> <?php } else {
                    echo '#';
                } ?>" class="btn erase-button <?php if (!$delete_file) echo 'disabled'; ?>" <?php if ($delete_file) { ?>onclick="return confirm('<?php echo lang_Confirm_del; ?>');"<?php } ?> title="<?php echo lang_Erase ?>"><i class="icon-trash"></i></a>
                                                </div>
