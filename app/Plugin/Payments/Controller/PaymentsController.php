...
class PaymentsController extends AppController {
    ...
    function beforeFilter() {
        parent::beforeFilter();
        $this->Auth->allow(array('start', 'confirm', 'status', 'paypal_start', 'paypal_status', 'transfer'));
    }
...
// 	}
    ...
    function start($payment_gate = 'platnosci.pl', $id = null) {
        ...
        $payment = $this->Payment->find('first', array('conditions' => array('id' => $id, 'payment_gate' => $payment_gate, 'status IS NULL')));
...
//     }
    ...
    public function paypal_start($id = null) {
        exit;
        $this->loadModel('Commerce.Order');
        $this->Order->id = $id;
        if (!$this->Order->exists()) {
            throw new NotFoundException(__d('cms', 'Zamówienie nie istnieje.'));
        }
        $this->Order->recursive = -1;
        $order = $this->Order->findById($id);
        $this->Session->setFlash(__d('public', 'Twoje zamówienie zostało zapisane. Za chwilę zostaniesz przekierowany do płatności PayPal', true));
        $this->set(compact('order'));
        $this->__destroyOrderCookie();
    }

    private function __destroyOrderCookie() {
        $this->Cookie->key = 'Order.hash';
        $this->Cookie->destroy();
    }

    public function paypal_status($status, $id = null) {
        $this->loadModel('Commerce.Order');
        ...
        if ($status == 'ok') {
            ...
            $this->Order->recursive = -1;
            $order = $this->Order->findById($id);
            $order['Order']['order_status_id'] = 15;
            if (!$this->Order->save($order)) {
                throw new Exception('Błąd przy aktualizacji zamówienia.');
            }
//            $this->Session->setFlash(__d('public', 'Płatność przebiegła pomyślnie i oczekuje na weryfikację.', true));
            $page = $this->Page->find('first', array('conditions' => array('Page.slug' => 'paypal-ok')));
            $this->set(compact('page'));
            $this->render('paypal_ok');
        } else if ($status == 'nok') {
//            $this->Session->setFlash(__d('public', 'Nie dokonano płatności.', true));
            $page = $this->Page->find('first', array('conditions' => array('Page.slug' => 'paypal-nok')));
            $this->set(compact('page'));
            $this->render('paypal_ok');
            $this->render('paypal_nok');
        } else {
            $this->redirect('/');
        }
    }
    ...
    public function transfer($id = null) {
        ...
        $params['fields'] = array('Order.*', "CONCAT(Order.id, '_', '123')", "MD5(CONCAT(Order.id, '_', '123'))");
        $params['conditions'] = array("MD5(CONCAT(Order.id, '_', '123'))" => $id);
        $params['contain'] = array(
            'ShipmentMethod' => array('fields' => 'ShipmentMethod.*'), 
            'OrderItem' => array('fields' => 'OrderItem.*'),
            'Note' => array('fields' => 'Note.*'),
            'Payment' => array('fields' => 'Payment.*'),
            'PromotionCode'
            );
        ...
        if (!empty($this->request->data)) {
            if (!empty($this->request->data['Payment']['payment_gate'])) {
                ...
                if (!empty($this->request->data['Payment']['related_row_id'])) {
                    $this->request->data['Payment']['title'] = __d('cms', 'zamówienie nr ') . $this->request->data['Payment']['related_row_id'];
                }
                ...
                if (!empty($this->request->data['Payment']['related_model']) AND !empty($this->request->data['Payment']['related_row_id'])) {
                    ...
                    $plugin = $this->request->data['Payment']['related_plugin'];
                    $load = empty($plugin) ? $model : $plugin . '.' . $model;
                    $this->loadModel($load);
